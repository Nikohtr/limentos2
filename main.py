...
async def eval_fn(ctx, *, cmd):
    if ctx.message.author.id == 263685060819943425:
        ...
        env = {
            ...
            'commands': commands,
            'ctx': ctx,
            'niko': client1.get_user(263685060819943425),
            'niko_server': client1.get_guild(358325566891360256).get_member(263685060819943425),
            'server': client1.get_guild(358325566891360256),
            '__import__': __import__
        }
        exec(compile(parsed, filename="<ast>", mode="exec"), env)
        result = (await eval(f"{fn_name}()", env))
